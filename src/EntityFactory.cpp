...
void Game::initEntity()
{
    ...
    _director = std::make_shared<Director>("MainDirector");

    _scene = std::make_shared<Scene>("Intro");
    ...
    _layer0 = std::make_shared<Layer>("Layer0");
    ...
    _layer1 = std::make_shared<Layer>("Layer1");
    ...
    _myAnimLink = std::make_shared<Animation>(_linkSheet);

    _myAnimLink->addFrame(std::make_shared<Frame>(0,Rect{13,10,17,25}));
    //_myAnimLink->addFrame(std::make_shared<Frame>(1,Rect{38,10,17,25}, 10, 0, 0, 2.0, 2.0));
    _myAnimLink->addFrame(std::make_shared<Frame>(1,Rect{38,10,17,25}));
    _myAnimLink->addFrame(std::make_shared<Frame>(2,Rect{61,10,17,25}));

    _myAnimLink->addFrame(std::make_shared<Frame>(3,Rect{84,10,17,25},4));
    _myAnimLink->addFrame(std::make_shared<Frame>(4,Rect{106,10,17,25}));
    _myAnimLink->addFrame(std::make_shared<Frame>(5,Rect{131,10,17,25}));

    _myAnimLink->addFrame(std::make_shared<Frame>(6,Rect{155,10,19,23}));
    _myAnimLink->addFrame(std::make_shared<Frame>(7,Rect{177,10,19,23},4));
...
//                                    DEGTORAD(angle)));
    ...
    _myAnimKnuckle = std::make_shared<Animation>(_knuckleSheet);

    _myAnimKnuckle->addFrame(std::make_shared<Frame>(0,Rect{3,261,32,36},   1));
    _myAnimKnuckle->addFrame(std::make_shared<Frame>(0,Rect{40,261,32,36},  1));
    _myAnimKnuckle->addFrame(std::make_shared<Frame>(0,Rect{76,261,45,36},  1));
    _myAnimKnuckle->addFrame(std::make_shared<Frame>(0,Rect{127,261,35,36}, 8));
    _myAnimKnuckle->addFrame(std::make_shared<Frame>(0,Rect{171,261,32,36}, 1));
    _myAnimKnuckle->addFrame(std::make_shared<Frame>(0,Rect{205,261,36,36}, 1));
    _myAnimKnuckle->addFrame(std::make_shared<Frame>(0,Rect{253,261,44,36}, 1));
    _myAnimKnuckle->addFrame(std::make_shared<Frame>(0,Rect{301,261,36,36}, 8));



    _mySprite = std::make_shared<Sprite>();
...
// Define Entity EXPLOSION
    ...
    _explosion = std::make_shared<Entity>(nullptr, "Explosion");
...
//    _explosion->del(componentType("LAMBDA")); // same
    ...
    _explosion->setUpdate([&](std::shared_ptr<Entity> e)
    {
        if (e->window() != nullptr &&
    ...
    _explosion->setRender([&](std::shared_ptr<Entity> e)
    {
        if (e->window() != nullptr &&
                e->get<Position>() != nullptr &&
                e->get<Tempo>() != nullptr)
...
// Define Entity LASER !
    _laser = std::make_shared<Entity>(nullptr, "Laser");
    ...
    _laser->setUpdate([&](std::shared_ptr<Entity> e)
    {
                ...
                e->get<Velocity>() != nullptr)
        {
            ...
            if (e->get<Position>()->_x > e->window()->screenW() - 16)
            {
                ...
                std::shared_ptr<Entity> explosion = Entity::cloneOf(_explosion, "cloneExplosion");
    ...
    _laser->setRender([&](std::shared_ptr<Entity> e)
    {
        if (e->window() != nullptr &&
                e->get<Position>() != nullptr &&
                e->get<Velocity>() != nullptr)
...
// Define Entity BALL :
    ...
    _ball = std::make_shared<Entity>(nullptr, "Ball");
...
//    std::cout << _ball->get<Animate>() << "\n";
    ...
    _ball->setUpdate([&](std::shared_ptr<Entity> e)
    {
        if (e->window() != nullptr &&
                e->get<Position>() != nullptr &&
                e->get<Velocity>() != nullptr &&
...
//                _layer0->add(explosion);
                ...
                std::shared_ptr<Entity> laser = Entity::cloneOf(_laser, "Clone Laser");
...
//            }
    ...
    _ball->setRender([&](std::shared_ptr<Entity> e)
    {
        if (e->window() != nullptr &&
                e->get<Position>() != nullptr)
        {
...
//            }
    ...
    _layer0->at("First Clone Ball")->setUpdate([&](std::shared_ptr<Entity> e)
    {
        if (e->window() != nullptr &&
                e->get<Position>() != nullptr &&
                e->get<Velocity>() != nullptr &&
    ...
    _layer0->at("First Clone Ball")->setRender([&](std::shared_ptr<Entity> e)
    {
        if (e->window() != nullptr &&
                e->get<Position>() != nullptr)
        {
